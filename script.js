import { SITE_CONTENT } from "./content.js?v=20260621-1";
...
function setLinks() {
  ...
  document.querySelectorAll('[data-role="download-link"]').forEach((link) => {
    if (SITE_CONTENT.meta.downloadUrl) {
  ...
  document.querySelectorAll('[data-role="sample-link"]').forEach((link) => {
    if (SITE_CONTENT.meta.sampleUrl) {
  ...
  document.querySelectorAll('[data-role="routine-simple-link"]').forEach((link) => {
    link.href = SITE_CONTENT.meta.routineSimpleUrl;
  });

  document.querySelectorAll('[data-role="routine-double-link"]').forEach((link) => {
    link.href = SITE_CONTENT.meta.routineDoubleUrl;
  });

  document.querySelectorAll('[data-role="other-book-link"]').forEach((link) => {
    link.href = SITE_CONTENT.meta.otherBookUrl;
  });
}
    ...
    schemaNode.textContent = JSON.stringify(
      {
        ...
        "@type": "Book",
        name: SITE_CONTENT.meta.bookName,
        author: {
          "@type": "Person",
...
function renderReviewPlaceholder() {
  return `
    <article class="testimonial-slide testimonial-placeholder">
      <h3>Opiniones disponibles próximamente</h3>
      <p class="testimonial-excerpt">Cuando el libro tenga sus primeras reseñas, esta sección mantendrá el mismo formato para incorporarlas sin tocar la estructura de la web.</p>
    </article>
  `;
}
...
function hydrate() {
  ...
  const reviewItems = SITE_CONTENT.amazonReviews.items;
  document.getElementById("testimonial-track").innerHTML = reviewItems.length
    ? renderTestimonials(reviewItems)
    : renderReviewPlaceholder();
  document.getElementById("testimonial-dots").innerHTML = reviewItems.length
    ? renderTestimonialDots(reviewItems)
    : "";
  ...
  document.getElementById("other-book-eyebrow").textContent = SITE_CONTENT.otherBook.eyebrow;
  document.getElementById("other-book-title").textContent = SITE_CONTENT.otherBook.title;
  document.getElementById("other-book-text").textContent = SITE_CONTENT.otherBook.text;
  document.getElementById("other-book-button").textContent = SITE_CONTENT.otherBook.button;
